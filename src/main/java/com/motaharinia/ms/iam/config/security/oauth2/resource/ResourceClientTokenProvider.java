...
@Component
@Slf4j
public class ResourceClientTokenProvider {
    /**
     ...
     */
    @Value("${app.security.public-key-client}")
    ...
    public ResourceClientTokenProvider(MessageSource messageSource) {
    ...
    public Authentication getAuthentication(String token) {
        ...
        Optional<LoggedInClientDto> loggedInClientDtoOptional = Optional.ofNullable(claims.get(CUSTOM_CLAIM_LOGGED_IN_CLIENT, LoggedInClientDto.class));
        if (loggedInClientDtoOptional.isEmpty()) {
            throw new RuntimeException("LoggedInClientDto is null.");
        }

        LoggedInClientDto loggedInClientDto = loggedInClientDtoOptional.get();

        Set<String> authoritySet = loggedInClientDto.getAuthoritySet();
        ...
        return new UsernamePasswordAuthenticationToken(loggedInClientDto, token, authorities);
    }
...
//                    .deserializeJsonWith(new JacksonDeserializer(new CustomObjectMapper))
                        .deserializeJsonWith(new CustomJacksonDeserializer(Maps.of(CUSTOM_CLAIM_LOGGED_IN_CLIENT, LoggedInClientDto.class).build(), new CustomObjectMapper()))
...
//                    .deserializeJsonWith(new JacksonDeserializer(new CustomObjectMapper))
     ...
     */
    @NotNull
    public Optional<LoggedInClientDto> getLoggedInDto() {
        ...
        if (authentication != null && !(authentication instanceof AnonymousAuthenticationToken) && authentication.getPrincipal() != null) {
            LoggedInClientDto loggedInClientDto = (LoggedInClientDto) authentication.getPrincipal();
            return Optional.of(loggedInClientDto);
        } else {
